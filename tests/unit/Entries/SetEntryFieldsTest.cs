...
namespace Laserfiche.Repository.Api.Client.Test.Entries
{
    public class SetEntryFieldsTest
    {
        ...
        public async Task AssignFieldValuesAsync_ReturnErrorStatusCode()
        {
            ...
            // ACT
            var response = await Assert.ThrowsAsync<ApiException>(async () => await client.EntriesClient.AssignFieldValuesAsync(repoId, entryId, fieldsToUpdate: request).ConfigureAwait(false));
