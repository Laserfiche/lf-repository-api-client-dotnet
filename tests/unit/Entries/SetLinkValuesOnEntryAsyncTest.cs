...
{
    public class SetLinkValuesOnEntryAsyncTest
    {
        [Fact]
        public async Task AssignEntryLinksAsync_200()
        {
            // ARRANGE
            ...
            var response = await client.EntriesClient.AssignEntryLinksAsync(repoId, entryId, new List<PutLinksRequest>()
            {
                new PutLinksRequest()
                {
                    TargetId = entryLinkInfo.TargetId,
                    LinkTypeId = 2,
                    CustomProperties = entryLinkInfo.LinkProperties
                },
                new PutLinksRequest()
                {
                    TargetId = entryLinkInfo2.TargetId,
                    LinkTypeId = 2,
            ...
            handlerMock.Protected().Verify(
               ...
               ItExpr.Is<HttpRequestMessage>(req =>
                  ...
                  && req.Content.ToString() == new StringContent(JsonConvert.SerializeObject(new List<PutLinksRequest>()
                    {
                        new PutLinksRequest()
                        {
                            TargetId = entryLinkInfo.TargetId,
        ...
        [Fact]
        public async Task AssignEntryLinksAsync_AnyOther()
        {
            // ARRANGE
            string baseAddress = "http://api.laserfiche.com/";
            string repoId = "repoId";
